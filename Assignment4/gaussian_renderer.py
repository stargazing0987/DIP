...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        J_proj: torch.Tensor = torch.stack([torch.tensor([[1.0/t[2],.0,-t[0]/t[2]**2],[.0,1.0/t[2],-t[1]/t[2]**2],],dtype=torch.float32)]*N).to(means3D.device)
        # Transform covariance to camera space
        # 使用求和约定简化实现
        covs_cam = torch.einsum('ij,njk,km->nim',R,covs3d,R.T) # (N, 3, 3)
        # Project to 2D
    ...
    def compute_gaussian_values(
        ...
        gaussian: torch.Tensor = torch.einsum('nhwi,nij,nhwj->nhw', dx, covs2D.inverse(), dx).mul(-.5).exp() ## (N, H, W)
